...
class SchroedingerTimeEvolution:
    def __init__(self, Delta=1e-3, deltatau=1e-4, runtime = 300, sigma_0=0.04, sigma_0_pyr=6.439e-12, k_0=20, pyr_inv_distance = 1/8, x_0=38.15e-12, m = 2.47*1.66e-27):
        ...
        self.T_k = int(runtime/self.deltatau * 1e-4)
        self.N = int(1/Delta)
        self.z_n = np.linspace(-1/2, 1/2, self.N)
        self.sigma_0 = sigma_0
        self.k_0 = k_0
        self.pyr_inv_distance = pyr_inv_distance
        self.sigma_0_pyr = sigma_0_pyr
        self.x_0 = x_0
        self.m = m
        self.a = self.x_0/self.pyr_inv_distance

    def psi_numerical_x(self, z):
        return np.exp((-z**2)/(4*self.sigma_0**2) + 1j*self.k_0*z)
    
    def psi_analytical_x_t(self, z, tau=0):
        #does not have intital velocity, but origin state is correct
        b = np.sqrt(self.sigma_0**2 + 1j*tau/2)
        return 1/b * np.exp(-z**2/(4*b**2))
    
    def psi_analytical_alt_x_t(self, z, tau=0):
        #found online, corresponds to the numerical solution
    ...
    def psi_pyramidal_inversion_x(self, z):
        #intital wave function of nitrogen atom in ammonia molecule (NH3). Used to showcase pyramidal inversion
    ...
    def v_pyramidal_inversion(self, z, omega_0=3.033e14):
        #the potential energy for the pyramidal inversion of the nitrogen atom in the ammonia molecule (NH3)
    ...
    def numerical_schroedinger_time_evolution(self, psi_func=psi_numerical_x, v_func=v_infsquare, normalize_every_step=True):
        ...
        psi_k_t = np.zeros((self.T_k, N), dtype=complex)
        psi_k_t[0] = psi_k

        if normalize_every_step == True:
    ...
    def analytical_schroedinger_time_evolution(self, psi_func=psi_analytical_alt_x_t):
        ...
        psi_k_t[0] = psi_k
        # print(f'calculating analytical time evolution: 0%', end='\r')
        for i in range(1, self.T_k):
...
if __name__ == "__main__":
    ...
    pyr_inv = SchroedingerTimeEvolution(Delta=1e-3, deltatau=1e-3, runtime=850000, pyr_inv_distance = 1/8)
    ...
    pyr_inv.animate_distribution(psi_k_t_pyr_inv, interval=1, save_gif=True, save_mp4=True, plot_type='pdf_pyr_inv', plot_every_nth_step=60)